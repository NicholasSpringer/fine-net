...
def sample_prints(identities_x, n_identities, n_prints_per_identity):
    ...
    sample_identities_indices = np.random.permutation(
    ...
    sample_identities_indices = tf.repeat(sample_identities_indices, n_prints_per_identity, axis=1)
    samples_prints_indices = np.empty([n_identities, identities_x.shape[1]])
    for i in range(n_identities):
        samples_prints_indices[i] = np.random.permutation(
            np.arange(identities_x.shape[1]))[:n_prints_per_identity]
    sample_indices = tf.stack([sample_identities_indices, samples_prints_indices], axis=2)
...
for i in range(N_BATCHES):
    sample_identities_x = sample_prints(identities_x_train, N_IDENTITIES, N_ANCHOR_PER_IDENTITY)
